function [w, infos] = cdadadelta(problem, in_options)
% Corrected Decentralized DADADELTA algorithm.%
...
if options.verbose > 0
    fprintf('C-DADADELTA: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
end
...
for ii=1:num_nod
    ...
    s_node(:,ii) = (1-options.beta)* grad_val_EX(:,ii).^2;
end
w_new_node=w_ex_node*options.net-grad_val_EX;
w=mean(w_new_node,2);
...
while (optgap > options.tol_optgap) && (epoch < options.max_epoch)
    ...
    for j = 1 : num_of_bachces
        ...
        for ii=1:num_nod
            ...
            r_node(:,ii) = options.beta * r_node(:,ii) + (1-options.beta)* grad_val_EX(:,ii).^2;
            grad_val_EX(:,ii) = grad_val_EX(:,ii)./(sqrt(r_node(:,ii)) + options.epsilon).*(sqrt(s_node(:,ii)) + options.epsilon);
            
            % update accumulated updates (deltas)
            s_node(:,ii) = options.beta * s_node(:,ii) + (1-options.beta)* grad_val_EX(:,ii).^2;
            
        end
        w_new_node=w_ex_node*(eye(num_nod)+options.net)-x_old*options.net2-(grad_val_EX-grad_val_old);
        %w_new_node=w_ex_node*options.net-grad_val_EX;
