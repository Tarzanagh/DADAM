function [w, infos] = cdadam(problem, in_options)
...
if options.verbose > 0
    fprintf('C-DADAM: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
end
...
for ii=1:num_nod
    ...
    m_node(:,ii) =  (1 - options.beta1).*grad_val_EX(:,ii);
    % update biased second raw moment estimate
    v_node(:,ii) = (1 - options.beta2).*(grad_val_EX(:,ii).^2);
    ...
    %
    grad_val_EX(:,ii) = m_hat_node(:,ii) ./ (sqrt(v_hat_node(:,ii)) + options.epsilon);
end
    ...
    for j = 1 : num_of_bachces
        ...
        for ii=1:num_nod
            ...
            grad_val_EX(:,ii)= problem.grad(w_ex_node(:,ii),sam_indice_node{ii});
                        ...
                        % update biased first moment estimate
                        m_node(:,ii) = options.beta1.*m_node(:,ii) + (1 - options.beta1).*grad_val_EX(:,ii);
                        % update biased second raw moment estimate
                        v_node(:,ii) = options.beta2.*v_node(:,ii) + (1 - options.beta2).*(grad_val_EX(:,ii).^2);
                        % compute bias-corrected fist moment estimate
                        m_hat_node(:,ii)= m_node(:,ii)./(1 - options.beta1^total_iter);
                        % compute bias-corrected second raw moment estimate
                        v_hat_node(:,ii)= v_node(:,ii)./(1 - options.beta2^total_iter);
