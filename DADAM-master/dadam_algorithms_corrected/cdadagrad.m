function [w, infos] = cdadagrad(problem, in_options)
%Corrected Decentralized DADAGRAD algorithm.
...
if options.verbose > 0
    fprintf('C-DADAGRAD: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
end
...
for ii=1:num_nod
    ...
    gradHist_node(:,ii) = grad_val_EX(:,ii).^2;
    grad_val_EX(:,ii) = grad_val_EX(:,ii)./ (sqrt(gradHist_node(:,ii)) + options.epsilon);
end
...
while (optgap > options.tol_optgap) && (epoch < options.max_epoch)
    ...
    for j = 1 : num_of_bachces
        ...
        for ii=1:num_nod
            ...
            sam_indice_node{ii} = randsample(indice_node{ii},options.batch_size);
            grad_val_EX(:,ii)= problem.grad(w_ex_node(:,ii),sam_indice_node{ii});
            gradHist_node(:,ii) = gradHist_node(:,ii)+ grad_val_EX(:,ii).^2;
            grad_val_EX(:,ii) = grad_val_EX(:,ii)./ (sqrt(gradHist_node(:,ii)) + options.epsilon);
        end
