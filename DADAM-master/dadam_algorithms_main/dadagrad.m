...
if options.verbose > 0
        fprintf('DADAGRAD: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
end
...
while (optgap > options.tol_optgap) && (epoch < options.max_epoch)
    ...
    for j = 1 : num_of_bachces
        ...
        for ii = 1 : num_nod  % can be run in parallel
            ...
            gradHist_node{ii} = gradHist_node{ii}+ grad_ii.^2;
            % update w
            ...
            w_node{ii} = w_consen_node - step * grad_ii ./ (sqrt(gradHist_node{ii}) + options.epsilon);
