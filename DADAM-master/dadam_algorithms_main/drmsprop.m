function [w, infos] = drmsprop(problem, in_options)
...
if options.verbose > 0
        fprintf('DRMSPROP: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
end
...
while (optgap > options.tol_optgap) && (epoch < options.max_epoch)
    ...
    for j = 1 : num_of_bachces
        ...
        for ii = 1 : num_nod  % can be run in parallel
            ...
            w_node{ii} = w_consen_node - step *  grad_ii./ (sqrt(r_node{ii}) + options.epsilon);
