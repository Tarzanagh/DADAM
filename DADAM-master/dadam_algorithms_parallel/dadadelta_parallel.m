...
if options.verbose > 0
    fprintf('DADADELTA-%s: Epoch = %03d, cost = %.16e, optgap = %.4e\n', options.sub_mode, epoch, f_val, optgap);
end
...
while (optgap > options.tol_optgap) && (epoch < options.max_epoch)
    ...
    for j = 1 : num_of_bachces
        ...
        parfor ii = 1 : num_nod  % can be run in parallel
            ...
            %
            if ~total_iter
                r_node{ii} = grad_node{ii}.^2;
                 d_node{ii} =  grad_node{ii}./(sqrt(r_node{ii}) + options.epsilon); 
                
            else
                % calculate accumulate squared gradient
                r_node{ii} = options.beta * r_node{ii} + (1-options.beta)* grad_node{ii}.^2;
                d_node{ii} =  grad_node{ii}./(sqrt(r_node{ii}) + options.epsilon).*(sqrt(s_node{ii}) + options.epsilon); 
            end
            % update accumulated updates (deltas)
            s_node{ii} = options.beta * s_node{ii} + (1-options.beta)* d_node{ii}.^2;
            
            % update w
            
            w_node{ii} = w_consen_node{ii} - d_node{ii};
